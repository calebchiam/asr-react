...
def generate_parse_command(input):
    return "deepspeech --model models/output_graph.pbmm --alphabet models/alphabet.txt --lm models/lm.binary --trie models/trie --audio {}".format(input)
...
def convert_and_parse_audio():
    if request.method == 'POST':
        ...
        if file and validate_file(file.filename, VALID_EXTS):
            ...
            out = system_call(generate_parse_command(output))
